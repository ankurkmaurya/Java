 ...
 * respective object lock. wait(): As the name suggests, it is a non-static
 ...
 * monitor (lock). It simply releases the lock and is mostly used for
 * inter-thread communication. It is defined in the object class, and should
 * only be called from a synchronized context.
 *
 * notify() : It sends a notification and wakes up only a single thread instead
 * of multiple threads that are waiting on the object’s monitor. notifyAll(): It
 * sends notifications and wakes up all threads and allows them to compete for
 * the object's monitor instead of a single thread.
 *
 *
 */
